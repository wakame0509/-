...
import pandas as pd
from distributed_shift_simulation_with_exclusion import run_distributed_simulation

st.set_page_config(page_title="統合ポーカー統計UI", layout="wide")
st.title("ポーカー統計ツール - 分担＆結果確認（6人対応版）")
...
st.markdown("#### 6人対応（他の4人にハンドを配って除外）")
exclude_others = st.checkbox("6人テーブル対応モード（4人分のハンドを除外）", value=True)

st.markdown("#### 結果ファイル名を入力（例：results_hp.csv）")
    ...
    if not selected_groups:
        ...
        with st.spinner("計算中...（数分〜十数分かかる場合があります）"):
            run_distributed_simulation(
                ...
                exclude_4players=exclude_others
            )
